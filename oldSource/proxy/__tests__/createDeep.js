...
import { unwrap } from '../../target/proxy';
...
describe('createDeep()', () => {
  ...
  describe('When type checkers disabled', () => {
    ...
    it('should not be type checked', () => {
      expect(isWrapped(createDeep(target))).toBe(false);
      expect(isWrapped(createDeep(target).list)).toBe(false);
    });
  });
  ...
  describe('When target is type checked already', () => {
    ...
    it('should skip checking children', () => {
      expect(unwrap(createDeep(target)).child).toEqual({
        val1: 1,
        val2: '2',
      });
    });
  ...
  describe('When target is valid and not type checked', () => {
    ...
    describe('When creating an object with already existing info', () => {
      ...
      beforeEach(() => {
        ...
        newTarget = unwrap(createDeep(object, { info }));
      });
